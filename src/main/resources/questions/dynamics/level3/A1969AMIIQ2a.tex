...
Figure \ref{fig:Dynamics_table_pulley_masses} shows the cross section of a smooth table ABCD, standing on the floor AD, with smooth pulleys attached at B and C. Particles of masses \vari{m\s{1}} $<$ \vari{m\s{2}} $<$ \vari{m\s{3}} are attached to a light, inelastic, inextensible string which passes over the pulleys as shown. The system is released from rest with the string taut when \vari{m\s{2}} is a height \vari{h\s{0}} from the floor.
\begin{enumerate}
	\item Find the initial acceleration of the system.
	\item The mass \vari{m\s{2}} hits the floor. Describe the subsequent motion of the system until the string again becomes taut. Find the following quantities, given that \value{m\s{1}}{1}{kg,} \value{m\s{2}}{3}{kg,} \value{m\s{3}}{5}{kg} and \value{h\s{0}}{0.45}{m}:
	\begin{enumerate}
		\item the velocity with which the system will move just after the string again becomes taut,
		\item the impulse on \vari{m\s{2}} at this instant, and the resulting loss of kinetic energy.
	\end{enumerate}
	\item Show that, when the system next comes instantaneously to rest, \vari{m\s{2}} will be $\frac{4}{9}$\var{h\s{0}} above the floor.
\end{enumerate}
