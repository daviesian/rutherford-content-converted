...
{\begin{enumerate}
	\item \exposition{A non-uniform rod of mass \valuedef{m}{200}{kg} and length \valuedef{l}{10.0}{m} has its centre of mass \valuedef{d}{4.0}{m} from one end. It is supported horizontally by two vertical ropes which are attached to points \valuedef{s}{1.0}{m} from each end.} \question[a]{Calculate the tension in each rope.}
	\item \exposition{The rod is raised by accelerating it at \valuedef{a}{0.20}{m\,s\sup{-2}} to a speed \valuedef{v}{0.60}{m\,s\sup{-1}}, which remains constant until the total height gained is \valuedef{h}{12.0}{m}.} 
	\begin{enumerate}
		\item \question[b]{Calculate the sum of the tensions in the ropes when the rod is accelerating}
		\item \question[c]{Calculate the sum of the tensions in the ropes when the rod is moving up at a steady speed}
		\item \question[d]{Calculate the power required when lifting the rod at \vari{v}}
		\item \question[e]{Calculate the distance for which the rod was accelerating and the distance for which it moved at \vari{v}}
	\end{enumerate}
	\item \question[f]{Using the above, or otherwise, find the work done raising the rod in terms of \vari{m}, \vari{v} and \vari{h}. Comment on the value.}
\end{enumerate}
}
...
\answer[a]{The tensions in the ropes are \valuedef{T_{1}}{735}{N} and  \valuedef{T_{2}}{1225}{N}}
	...
	\begin{equation*}
	 ...
	 \end{align*}
	 \answer[b]{The total of the tensions in the ropes when the rod is accelerating is \valuedef{T_{tot}}{2000}{N}.}
		...
		\answer[c]{The total of the tensions in the ropes when the rod is moving at a constant speed is \valuedef{T_{tot}}{1960}{N}.}
...
This could also have been found another way; since the speed is constant, the power is the total energy required divided by the time taken to use that energy. Calling the distance moved at the constant speed \vari{s_{2}}:
	...
	\end{equation*} 
	as before.
	\answer[d]{The power required when lifting the rod at \vari{v} is \valuedef{P}{1176}{W}.}
...
and then \vari{s_{2}} is simply \vari{h - s_{1}} $=$ \valuedef{h - \frac{v^{2}}{2a}}{11.1}{m}.
	...
	\answer[e]{The distance moved whilst accelerating is \valuedef{s_{1}}{0.9}{m}, and that moved whilst at a constant speed is \valuedef{s_{2}}{11.1}{m}.}
...
\item We need to use the formula \vari{W} $=$ \valuedef{\vtr{F} \cdot \vtr{x}}{Fd}{} where \vari{F} is the magnitude of the force \vari{\vtr{F}} and d is the distance moved in the direction of the force. There are two sections to the motion; those when the rod is accelerating and the tension is \vari{T_{\text{tot}}} and that when it is moving at a constant speed, with tension $T = T_{1} + T_{2}$, and the total work is the sum of these two.
	...
	\answer[f]{The work done while raising the rod is \valuedef{W}{\frac{1}{2}mv^{2} + mgh}{}. This is the sum of the kinetic and gravitational potential energy the rod gained as it was lifted.} The work done by the ropes was the only way the rod could have gained energy, so an energy argument could have been used here instead.
\end{enumerate}}
\end{problem}