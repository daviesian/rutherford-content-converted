...
\begin{enumerate}
	\item \question[a]{Find \vari{v^{2}} in terms of \vari{l}, \vari{x} and \vari{g}.}
	\item \question[b]{Derive an expression for the horizontal component of the momentum of the section of chain within the tube as a function of \vari{\rho}, \vari{l} and \vari{v}.}
	\item  \exposition{Show further that the horizontal component of the force on the chain is:
	\begin{equation*} 
	\frac{2g \rho}{l(8 + \pi)} \left( 9l^{2} - 6lx - 2x^{2} \right) 
	\end{equation*}}
\end{enumerate}

\stress{Hint:}{\begin{enumerate}
	\item \hinta[a]{It is easiest to consider equating the kinetic energy gained after moving a distance \vari{x} to the gravitational potential energy (GPE) lost in the same distance.}
	\item \hinta[c]{Consider a small element, and integrate %over the whole curved section 
to find the total horizontal momentum of the chain.}% in the tube.
	\item \hinta[c]{The rate of change of horizontal momentum in the tube is equal to the sum of the two horizontal forces acting on the chain plus the momentum flux into the tube.}
\end{enumerate}}
}
