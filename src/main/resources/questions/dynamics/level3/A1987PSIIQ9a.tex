...
{A metal block of mass \vari{m} rests on a rough wooden surface. A thread is attached to the right side of the block and a tension is applied to cause it to move. The motion is opposed by a constant frictional force \vari{F\s{r}} between the block and the surface.
\begin{enumerate}
	\item Draw a diagram to show all the forces on the block immediately after motion commences.
	\item When the tension in the thread is \vari{T}, the acceleration of the block is measured to be \vari{a}. Calculate the new acceleration, \vari{a\s{n}}, if the tension is increased to \vari{\frac{5}{4}T}.
