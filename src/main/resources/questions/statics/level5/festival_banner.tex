...
%% CONCEPTS: forces, newtoni, trig, vectors
%% VIDEOS: 
%% LEVEL: 5
...
The wire is attached on one side to the buliding's roof, and on the other side to a crane, of height \vari{h}. The banner needs to be hung at a distance \vari{y} vertically below the level of the building's roof, and horizontal distance \vari{x} from the building without the crane.}
\begin{enumerate}
	...
	\item \question[c]{Rearrange to find \vari{x} and \vari{y} in terms of \vari{h} and \vari{\lambda}. Explain the behaviour of these equations as \vari{\lambda} goes to $\sqrt{1 + \frac{h^2}{l^2}}$ and to infinity (assume \vari{h} remains finite).}
	\item \question[d]{why are there no solutions for $\lambda < \sqrt{1 + \frac{h^2}{l^2}}$?}
	\item \question[e]{Assuming $\lambda > \sqrt{1 + \frac{h^2}{l^2}}$ and \valuedef{\lambda}{2}{} and the mass of the banner is \quantity{10}{kg}, find the tension \vari{T} in the banner.}
	}
\end{enumerate}
}
{Written by Zephyr Penoyre for the RSPP, edited by MC.}
{\answer[a]{}
\begin{enumerate}
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{../../../figures/Statics_banner.eps}
	\caption{}
...
\end{figure}
	...
	\begin{equation}	\lambda l = \sqrt{x^{2} + y^{2}} + \sqrt{{(l - x)}^{2} + {(h + y)}^{2}}  \end{equation}
	 ...
	 \answer[b]{\begin{equation}	y = x \sqrt{\lambda^2 - 1} 
	 ...
	 \item Rearranging equation (\ref{eqn:similar}) we get:
	  ...
	  \answer[c]{ \begin{equation}	x = \frac{1}{2} \left(  l - \frac{h}{\sqrt{\lambda^2 -1}} \right)  \end{equation}
	   ...
	   As $\lambda$ goes to infinity,  $\sqrt{\lambda^2 -1}$ goes to $\lambda$ which is much greater than $h$, thus x goes to $\frac{l}{2}$, i.e. the banner sits half way between the buildings. At the same time $y$ goes to $\frac{\lambda l}{2}$, which means the banner hangs at half the height of the total length of wire. Both of these results correspond to the banner sitting directly in the middle of the wire.
	     ...
	     \item \answer[d]{For $\lambda < \sqrt{1 + \frac{h^2}{l^2}}$, the distance between the two ends of the wire ($\sqrt{l^2 + h^2}$), i.e. from one rooftop to the top of the crane, is greater than the length of the wire $\lambda l$. This is at odds with the earlier assumption that the wire is inextensible and thus is a physical impossibility.}
