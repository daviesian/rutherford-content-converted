...
The units are \quantity{}{kg\,m\,s\sup{-1}}, though \quantity{}{N\,s} (Newton seconds) are also acceptable since they are equivalent.
	\item If we can assume the force to be constant, then there is a constant acceleration and so the SUVAT equations can be used. The equation we want is \valuedef{v^{2}}{u^{2} + 2as}{} and we can either solve it for \vari{a} and then use Newtons Second Law, \valuedef{\vtr{F}}{m\vtr{a}}{}; or substitute for \vari{a} initially:
\begin{equation*} 
