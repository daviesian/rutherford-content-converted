...
\begin{problem}[Inelastic collision in the ZMF] %A2-2
 {
 \begin{enumerate}
\item  \exposition{A collision occurs between two (non-relativistic) bodies of equal mass \vari{m} and velocity vectors \vari{\vtr{v}_{1}} and \vari{\vtr{v}_{2}}.}  \question[a]{How much kinetic energy is available for conversion to other forms of energy?}
 \item \exposition{A particle of mass \vari{m} travelling with speed \vari{V} along the \vari{+x} direction collides elastically with a stationary particle of mass \vari{2m}.The particle of mass \vari{m} is deflected through an angle of \vari{30^{\circ}}.}   \question[b]{What are the final velocity vectors of the two particles in the Laboratory frame?}
\end{enumerate} 
\exposition{Your answer should be illustrated by appropriate diagrams in both the laboratory and zero momentum frames.}}
...
\end{equation*}
 which can be rewritten as \answer[a]{\vari{\frac{1}{4}m|\vtr{v}_{1}-\vtr{v}_{2}|^2}}.
...
\answer[b]{The velocity of the particle of mass \vari{2m}, is equal to \vari{\frac{2v}{3}\cos{(\phi)}} and the velocity of the particle of mass \vari{m}, is equal to \vari{\frac{2\sqrt{3}}{9}(1-2\cos{(2\phi)})v}}
