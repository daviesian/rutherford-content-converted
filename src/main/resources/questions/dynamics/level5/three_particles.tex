...
%% CONCEPTS: momentum, energy, newtonii, momentum, resolving_vectors, vectors, eq_of_motion_diff, relative_motion
%% VIDEOS: 
%% LEVEL: 5
...
{Three identical particles \vari{A}, \vari{B}, and \vari{C} are moving in a plane and, at time \vari{t}, their position vectors with respect to an origin \vari{O} are:
\begin{align*} 
...
3t &= 9 \\ 
t &= 3 
\end{align*} Of course, in general we would need to check the \vari{y}-components were equal to show a collision occurs (the lines may cross, but the particles don't have to have the same \vari{y}-coordinate at the same time the \vari{x}-coordinates are the same); but here we know that it does.
