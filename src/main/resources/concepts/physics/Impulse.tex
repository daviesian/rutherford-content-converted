...
\caption{The graph of a force that changes over time from an initial value of \vari{F\s i} to \vari{F_f} over a time of \vari{t} seconds.  The impulse that the object experiences can be calculated using the area under the line of graph, shown in pink.}  \label{impulse2}
\end{wrapfigure} 
...
\noindent We can calculate the area under the force line by adding up the areas of a series of small rectangles of width \vari{\d t} from \value{t}{0}{} to \value{t}{t\s f}{}. Using figure \ref{impulse2} we can see that the area of a small rectangle is given by the height of the rectangle \vari{\vtr F} multiplied by the width of the rectangle \vari{\d t}.  The instantaneous impulse, \vari{\d{\vtr I}} at time \vari{t} is therefore
\begin{equation}
\d{\vtr I} = {\vtr F}\d t
\end{equation}
