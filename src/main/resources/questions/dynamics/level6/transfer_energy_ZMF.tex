...
To plot this function we must consider the value in the limit of very small and very large \vari{m_2}. As \vari{m_2 \to 0} the denominator remains finite while the numerator goes to 0. Therefore the value of the final particle's kinetic energy goes to zero as the mass of the middle particle goes to zero. We must also consider its kinetic energy as \vari{m_2 \to \infty}. Since the numerator goes like \vari{{m_2}^2} and the denominator goes like \vari{{m_2}^4}, the function goes to zero in this limit. Therefore we expect a maximum at some value of \vari{m_2}. Careful differentiation at \valuedef{m_2}{0}{kg} also shows that the gradient must go to zero here. See Figure \ref{fig:Tripos_Elastic_ZMF_2} for the plot. 
