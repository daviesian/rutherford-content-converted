...
{\exposition{If a body is undergoing constant acceleration, then its acceleration can be defined as $a~=~\frac{\textrm{Change in speed}}{\text{Time taken to change speed}} =$ \vari{\frac{v - u}{t}} where \vari{u} is the initial speed and \vari{v} the final speed of the body in the time period \vari{t}.}
\begin{enumerate}
	\item \question[a]{In terms of \vari{u}, \vari{v} and \vari{t} and by using the definition of average speed; what distance, \vari{s}, does the body travel in a time \vari{t}?}
	\item\exposition{Using the equation \valuedef{a}{\frac{v - u}{t}}{} and the equation derived in (a) for \vari{s}:}
	\begin{enumerate}
		\item \question[b]{Write an equation for \vari{s} that does not involve \vari{v}}
		\item \question[c]{Write an equation for \vari{s} that does not involve \vari{u}}
		\item \question[d]{Write an equation for \vari{s} that does not involve \vari{t}}
	\end{enumerate}
	\question[e]{Bonus: The two equations in (b) and (c) can be found graphically from a speed-time graph. If at \valuedef{t}{0}{s} the body has speed \vari{u} and at time \vari{t} has speed \vari{v}, plot a graph showing this motion and explain graphically the origins of the equations for \vari{s}.}
\end{enumerate}
}
