...
\Rightarrow \mu\s{crit}&= 2\cot{\alpha}
 ...
 When the actual value of \vari{\mu} is greater than this critical value, the block will topple before it slides, as clearly increasing the coefficient of friction will make sliding more difficult. 
 	\begin{enumerate}
 		\item \answer[c]{If $\mu = \cot{\alpha}$, then $\mu < \mu\s{crit}$ so equilibrium will be broken by the ladder sliding.}
		\item \answer[d]{If $\mu = 4\cot{\alpha}$, then $\mu > \mu\s{crit}$ so equilibrium will be broken by the ladder toppling.}
	\end{enumerate}
\end{enumerate}
}
\end{problem}