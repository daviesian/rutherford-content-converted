...
%% QUESTIONS: elastic_circular_motion, firing_the_rockets, roundabout, fairground_ride, starting_w_basics, particles_on_string, head_on_collision, on_ice, walking_up_hill, hammer_nail, spouting_can, powering_up, braking_a_car
...
figure~\ref{fig:project-vectors}(a).  The projection of \vari{\vtr{u}} onto the \stress{direction} of $\vtr{v}$ is $u \cos\theta$.
\begin{figure}[h!]
\centering
...
\end{figure}
The projection of \vari{\vtr{u}} onto the coordinate axes is shown in figure~\ref{fig:project-vectors}(b), these projections of magnitude \vari{x} and \vari{y} respectively, form an ordered pair \vari{(x,y)} that is called the coordinate representation or Cartesian form of the vector.  In the example \valuedef{x}{3}{} and \valuedef{y}{1}{}.
\qq{What is the angle $\theta$ in the figure?}{\valuedef{\theta}{\arctan(1/3)}{}.  Give other equivalent forms for $\theta$.}
\subsection*{Modulus, magnitude, or length of vectors; unit vectors}
Pythagoras in figure~\ref{fig:project-vectors}(b) tells us the length of the vector is  \vari{|\vtr{u}|} $=$ \valuedef{u}{\sqrt{x^2 + y^2}}{}.
