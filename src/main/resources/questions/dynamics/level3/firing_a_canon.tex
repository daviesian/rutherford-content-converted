...
{\begin{enumerate}
	\item \question[a]{A gun of mass \valuedef{M}{1000}{kg} stands on horizontal ground and can move horizontally against a recoil mechanism. With its barrel raised at an angle of \valuedef{\theta}{60^{\circ}}{} to the ground, it fires a projectile of mass \valuedef{m}{20}{kg} with a speed \valuedef{u}{306}{m\,s\sup{-1}} \stress{relative to the barrel}. Calculate the horizontal impulse on the gun and the total kinetic energy produced by the explosion which fired the projectile. Clearly state the units in which both quantities are measured.}
	...
	\item \question[c]{Show that the time taken for the gun to recoil through this distance is \quantity{\frac{1}{6}}{s}.}
\end{enumerate}
}
{\stress{Used with permission from UCLES, A Level Applied Mathematics, June~1972, Paper~2, Question~2.}}
\answer[a]{The first key thing to realise is that the speed of the projectile is given relative to the barrel, not the ground (since the same impulse acts on two things at rest, the ground frame is the zero momentum frame [ZMF]). Resolving the speed to the horizontal, \vari{u \cos(\theta)}, and then drawing a diagram of the gun and projectile as particles will aid in finding the horizontal speeds in the ZMF, as in Figure \ref{fig:Dynamics_gun_speeds}. The barrel is moving away from the projectile, and so the speed of the projectile will be less than \vari{u \cos(\theta)} by some amount \vari{v}, the speed of the gun recoiling.
...
\answer[b]{If the force is constant, then the acceleration is constant and so we can use Newton's Second Law and the SUVAT equation \valuedef{v^{2}}{u^{2} + 2as}{}. Since right is our positive direction, we have \valuedef{s}{-r}{,} \valuedef{u}{\frac{-I}{M}}{} and \valuedef{v}{0}{m\,s\sup{-1}} so:
\begin{align*} 
...
\answer[c]{Again we can use a SUVAT equation to find the time taken, \valuedef{s}{\left( \frac{u + v}{2} \right)t}{} with \vari{s}, \vari{u} and \vari{v} as before:
\begin{align*} 
